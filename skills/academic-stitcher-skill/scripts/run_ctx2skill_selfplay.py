...
def main(argv: list[str] | None = None) -> int:
    ...
    api_key_value = os.getenv(args.api_key_env)
    ...
    command_prefix = [sys.executable, selfplay_cmd] if str(selfplay_cmd).lower().endswith(".py") else [selfplay_cmd]
    ...
    # The downstream CLI expects the standard OpenAI-compatible names.  Allow
    # callers to keep credentials in custom environment variables without
    # exposing the value on the command line or in logs.
    if api_key_value:
    ...
    clean_run = "- Run status: completed\n" in summary_text
    if completed.returncode != 0 or credential_error or summary_errors or not run_path.exists() or not clean_run:
        return 1
    return 0
