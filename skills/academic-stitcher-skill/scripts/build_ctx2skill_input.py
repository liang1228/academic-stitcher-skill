...
def resolve_pack_path(root: Path, rel_path: str) -> Path:
    """Resolve a manifest path and require it to stay inside the skill root."""
    candidate = Path(rel_path)
    if candidate.is_absolute():
        raise SystemExit(f"Manifest path must be relative: {rel_path}")
    resolved = (root / candidate).resolve()
    try:
        resolved.relative_to(root)
    except ValueError as exc:
        raise SystemExit(f"Manifest path escapes skill root: {rel_path}") from exc
    if not resolved.is_file():
        raise SystemExit(f"Manifest path is not a file: {rel_path}")
    return resolved
...
def main(argv: list[str] | None = None) -> int:
    ...
    for rel_path in rel_paths:
        try:
            path = resolve_pack_path(root, rel_path)
