...
def load_records(path: Path) -> tuple[list[dict], list[str]]:
    ...
    if records and task_count == 0:
        errors.append("run contains zero task results")
...
def summarize(records: list[dict], errors: list[str]) -> str:
    ...
    if errors:
        run_status = "not completed" if not records or total_tasks == 0 else "completed with warnings"
...
def main() -> int:
    ...
    return 1 if args.fail_on_warning and (errors or not clean_run) else 0
